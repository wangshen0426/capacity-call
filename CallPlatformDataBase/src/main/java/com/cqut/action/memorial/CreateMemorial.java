...
public class CreateMemorial {
	...
	public void exe() {
		if (userId != null && memorialTime != null && memorialDate != null) {
			StringBuilder memorialStb = new StringBuilder();
			memorialStb.append(System.currentTimeMillis());
			memorialStb.append(userId);

			String sql = "insert into memorial (memorialId, memorialDate, memorialPosition, memorialText, memorialTime, userId) VALUES (?,?,?,?,?,?)";
			Object[] param = { memorialStb.toString(), memorialDate,
					memorialPosition, memorialText, memorialTime, userId };
