...
public class Node <T>{ 
    ...
    /** 节点的关键字 */ 
    protected List<Entry<Comparable, T>> entries; 
    ...
    public Node(boolean isLeaf) { 
        this.isLeaf = isLeaf; 
        entries = new ArrayList<Entry<Comparable, T>>(); 
    ...
    public boolean insertOrUpdate(Comparable key, T obj, BPlusTree tree){ 
    	boolean result = false;
        //如果是叶子节点 
        if (isLeaf){ 
            //不需要分裂，直接插入或更新 
            if (contains(key) || entries.size() < tree.getOrder()){ 
            	result = insertOrUpdate(key, obj); 
                if (parent != null) { 
                    //更新父节点 
            ...
            //需要分裂   
                ...
                //分裂成左右两个节点 
                Node<T> left = new Node<T>(true); 
                Node<T> right = new Node<T>(true); 
                //设置链接 
                if (previous != null){ 
                ...
                //如果是根节点     
                    ...
                    isRoot = false; 
                    Node<T> parent = new Node<T>(false, true); 
                    tree.setRoot(parent); 
                    left.setParent(parent); 
                    right.setParent(parent); 
        ...
        //如果不是叶子节点 
            ...
            //如果key小于等于节点最左边的key，沿第一个子节点继续搜索 
            if (key.compareTo(entries.get(0).getKey()) <= 0) { 
                return children.get(0).insertOrUpdate(key, obj, tree); 
            //如果key大于节点最右边的key，沿最后一个子节点继续搜索 
            }else if (key.compareTo(entries.get(entries.size()-1).getKey()) >= 0) { 
            	return children.get(children.size()-1).insertOrUpdate(key, obj, tree); 
            //否则沿比key大的前一个子节点继续搜索 
            }else { 
                for (int i = 0; i < entries.size(); i++) { 
                    if (entries.get(i).getKey().compareTo(key) <= 0 && entries.get(i+1).getKey().compareTo(key) > 0) { 
                    	return children.get(i).insertOrUpdate(key, obj, tree); 
                    } 
                }    
            } 
        }
    ...
    protected void updateInsert(BPlusTree tree){ 
        ...
        if (children.size() > tree.getOrder()) { 
            //分裂成左右两个节点 
            Node<T> left = new Node<T>(false); 
            Node<T> right = new Node<T>(false); 
            //左右两个节点关键字长度 
            int leftSize = (tree.getOrder() + 1) / 2 + (tree.getOrder() + 1) % 2; 
            int rightSize = (tree.getOrder() + 1) / 2; 
            ...
            for (int i = 0; i < leftSize; i++){ 
                ...
                left.getEntries().add(new SimpleEntry<Comparable,T>(children.get(i).getEntries().get(0).getKey(), null)); 
    ...
    /** 调整节点关键字*/ 
    protected  void validate(Node <T> node, BPlusTree tree) { 
        ...
        if (children.size() < tree.getOrder() / 2 || children.size() < 2) { 
            if (isRoot) { 
                ...
                // 如果后节点子节点数大于M / 2并且大于2，则从其处借补 
                    ...
                    //后叶子节点首位添加到末尾 
                    Node<T> borrow = next.getChildren().get(0); 
                ...
                // 否则需要合并节点 
                    ...
                    if (previous != null  
                        ...
                        for (int i = previous.getChildren().size() - 1; i >= 0; i--) { 
                            Node<T> child = previous.getChildren().get(i); 
                    ...
                    // 同后面节点合并 
                        ...
                        for (int i = 0; i < next.getChildren().size(); i++) { 
                            Node<T> child = next.getChildren().get(i); 
                            children.add(child); 
                            child.setParent(this); 
                        } 
            ...
            if (isRoot) { 
                ...
                if (entries.size() > tree.getOrder() / 2 && entries.size() > 2) { 
                    ...
                    if (previous != null  
                        ...
                        Entry<Comparable, T> entry = previous.getEntries().get(size - 1); 
                    ...
                    //如果自身关键字数小于M / 2，并且后节点关键字数大于M / 2，则从其处借补    
                        ...
                        Entry<Comparable, T> entry = next.getEntries().get(0); 
    ...
    protected boolean contains(Comparable key) { 
        for (Entry<Comparable, T> entry : entries) { 
    ...
    /** 插入到当前节点的关键字中*/ 
    protected boolean insertOrUpdate(Comparable key, T obj){ 
        Entry<Comparable, T> entry = new SimpleEntry<Comparable, T>(key, obj); 
        //如果关键字列表长度为0，则直接插入 
        if (entries.size() == 0) { 
            entries.add(entry); 
            return true; 
    ...
    public List<Entry<Comparable, T>> getEntries() { 
        return entries; 
    } 
 
    public void setEntries(List<Entry<Comparable, T>> entries) { 
